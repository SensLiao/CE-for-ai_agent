...
<p align="center"><img src="docs/cli-session.png" alt="终端会话：CLI help 列出八个命令组、scan --help、实时进程列表，以及 104 个测试在 0.53 秒内通过" width="100%"></p>

<p align="center"><sub>截自 Windows 11 上的真实运行，且**没有安装 Cheat Engine**——进程枚举走的是原生 Windows API，这正是独立路径存在的意义。104 个测试用例耗时 0.53 秒，且不需要任何目标进程，因此可以安全地放进 CI。</sub></p>

这段输出里有两点值得注意。`process list` 在没有附加任何目标、也没有提权的情况下就能工作，因为"枚举"和"附加"是两种不同的权限——见[权限矩阵](#-权限矩阵)。以及每个命令组最终都落到接受 `--json` 的叶子命令上，所以操作者手动驱动的那个会话，和智能体通过解析 stdout 驱动的会话，是同一个。
